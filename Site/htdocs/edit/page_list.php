...
// This page displays the list of pages screen
	...
	checkSecurity($pdo, $user, "Page - List Pages");
	
	try {
		// SQL to retrieve page list:
		$q = 'SELECT id, title, dateUpdated FROM SIT_pages';
		$stmt = $pdo->prepare($q);
		$r = $stmt->execute();
		
		// Set Page Title:
		$smarty->assign('title', 'CMS Control Panel - Page List');
